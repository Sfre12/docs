...
SetOption132<a class="cmnd" id="setoption132"></a>|When MQTT TLS is enabled, forces fingerprint validation of server identity instead of checking the identify against a certificate authority (CA)<BR>`1` = Fingerprint, `0` = CA (In some cases, since Version V9.5 and higher, it is necessary to activate it to have success with Telegram messages)
